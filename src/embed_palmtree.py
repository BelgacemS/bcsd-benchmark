...
# en local le script est dans src/, sur la VM il est a la racine du workspace
...
# detection auto GPU/CPU (configurable via --device)
...
def detect_device(choice="auto"):
    if choice == "cuda" or (choice == "auto" and torch.cuda.is_available()):
        ...
        print("GPU detecte, mode CUDA")
        return torch.device("cuda")
    palmtree_config.USE_CUDA = False
    print("Mode CPU")
    return torch.device("cpu")

# defaut CPU, re-detecte dans __main__ avec le flag --device
...
class PalmTreeEncoder:
    ...
    def __init__(self, model_path, vocab_path, batch_size=512, device=None):
        ...
        print(f"  PalmTree pret ({self.device})")
...
def run_plan_b(cfg, disasm_dir, emb_base):
    # plan B : BERT baseline (HuggingFace), pas entraine sur l'asm
    print("\nPlan B: BERT baseline")
    try:
        from transformers import BertTokenizer, BertModel
    except ImportError:
        print("  transformers pas installe, on passe au plan C")
        return False
    ...
    def encode_file(jf):
        ...
        for func in funcs:
            ...
            if not insns:
                continue
            text = " ; ".join(normalize_instruction(m, o) for m, o in insns)
        ...
        if not func_embs:
            return meta, None, []
        return meta, np.stack(func_embs), names

    return run_approach("bert_baseline", encode_file, cfg, disasm_dir, emb_base)


def run_plan_c(cfg, disasm_dir, emb_base):
    # plan C : Word2Vec sur les mnemonics
    print("\nPlan C: Word2Vec baseline")
    try:
        from gensim.models import Word2Vec
    except ImportError:
        print("  gensim pas installe, impossible de continuer")
        sys.exit(1)
    ...
    for jf in json_files:
        ...
        for func in data.get("functions", []):
            for m, o in func.get("instructions", []):
                sentences.append(normalize_instruction(m, o).split())

    print(f"  entrainement sur {len(sentences)} instructions")
    w2v = Word2Vec(sentences, vector_size=100, window=5, min_count=1, workers=4, epochs=20)
    ...
    def encode_file(jf):
        ...
        for func in funcs:
            ...
            insn_vecs = []
            for m, o in insns:
                toks = normalize_instruction(m, o).split()
                vecs = [w2v.wv[t] for t in toks if t in w2v.wv]
                if vecs:
                    insn_vecs.append(np.mean(vecs, axis=0))
            if not insn_vecs:
                continue
            names.append(func["name"])
            func_embs.append(np.mean(insn_vecs, axis=0))

        if not func_embs:
            return meta, None, []
        return meta, np.stack(func_embs), names

    return run_approach("word2vec_baseline", encode_file, cfg, disasm_dir, emb_base)
...
if __name__ == "__main__":
    ...
    # fallback : PalmTree -> BERT -> Word2Vec
    ok = run_palmtree(cfg, disasm_dir, emb_base)
    if not ok:
        print("PalmTree a echoue, on passe au plan B")
        ok = run_plan_b(cfg, disasm_dir, emb_base)
    if not ok:
        print("Plan B a echoue, on passe au plan C")
        run_plan_c(cfg, disasm_dir, emb_base)
