#!/usr/bin/env python3
# src/disasm_jtrans.py
# desassemblage BB-level pour jTrans
# scan directement data/binaries/ (pas besoin de manifest)
...
# mapping des noms de dossier lang vers le nom normalise
...
def extract_blocks(func):
    # extrait les blocs de base tries par adresse
    blocks = []
...
def parse_bin_path(bin_path, bin_dir):
    ...
    # binaries/compiler/arch/optim/dataset/problem/lang__impl_NN
...
def process_one(bp, bin_dir, out_dir, cfg_disasm):
    # worker pour le pool multiprocessing : traite un binaire, retourne (status, nb_funcs)
    fsize = os.path.getsize(bp)
    if fsize > MAX_FILE_SIZE:
        return "skip", 0

    try:
        comp, arch, opt, dataset, problem, lang, src_id = parse_bin_path(bp, bin_dir)
    except Exception:
        return "err", 0
...
if __name__ == "__main__":
    ...
    run_disasm(cfg, max_workers=args.workers)
